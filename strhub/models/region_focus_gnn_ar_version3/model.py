...
class MultiRegionCSLRModel(nn.Module):
    ...
    def encode(self, poses):
        ...
        for i, (region_name, joint_indices) in enumerate(self.regions.items()):
            ...
            region_feat = self.region_models[region_name].encode(region_poses)  # [B, T/4, region_feature_dim]
            
            # Add region position embedding
            # region_feat = region_feat + self.region_pos_emb[i]  # [B, T/4, region_feature_dim]
        ...
        frame_features = torch.cat(region_features, dim=-1)  # [B, T/4, num_regions*region_feature_dim]
    ...
    def get_context(self, memory, num_step=None):
        if num_step is None:
            num_step = self.max_output_len #create context for token != eos bos
        bs = memory.shape[0]
        pos_queries = self.pos_queries[:, :num_step].expand(bs, -1, -1)
        # No prior context, so input is just <bos>. We query all positions.
        tgt_in = pos_queries.clone()
        tgt_out = self.decode(tgt_in, memory, tgt_query=pos_queries)
        context = pos_queries.clone()
        context[:, 1:] = tgt_out[:, :-1]
        # pos_queries[:, 1:] += tgt_out[:, :-1]
        # context[:, 1:] += tgt_out[:, :-1]
        return context
    def forward(self, tokenizer: Tokenizer, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        for i in range(num_steps):
            ...
            logits.append(p_i)
            hidden.append(tgt_out)
            if j < num_steps:
                # print(tgt_in[:, j].shape, tgt_out.shape)
                # input()
                tgt_in[:, j] += tgt_out.squeeze(1)
        ...
        if self.refine_iters:
            ...
            for i in range(self.refine_iters):
                tgt_in = hidden
                tgt_padding_mask = (torch.cat([bos, logits[:, :-1].argmax(-1)], dim=1) == tokenizer.eos_id).int().cumsum(-1) > 0
                tgt_out = self.decode(
