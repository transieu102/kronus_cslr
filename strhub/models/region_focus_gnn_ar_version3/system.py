...
class MultiRegionCSLRSystemVersion3(CrossEntropySystem):
    ...
    def fusion_forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        return self.model.fusion_forward(self.tokenizer, images, max_length)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        tgt_in = tgt[:, :-1]
        tgt_out = tgt[:, 1:]
        context= self.model.get_context(memory, num_step = tgt_in.shape[1])
        ...
        region_loss_weight = 1/len(self.model.regions)
        for i, perm in enumerate(tgt_perms):
            tgt_mask, query_mask = self.generate_attn_masks(perm)
            # print(tgt_mask.shape, query_mask.shape, tgt_in.shape, context.shape)
            # input()
            out = self.model.decode(context, memory, tgt_mask, tgt_padding_mask, tgt_query_mask=query_mask)
            logits = self.model.out_proj(out).flatten(end_dim=1)
            loss += n * F.cross_entropy(logits, tgt_out.flatten(), ignore_index=self.pad_id)
            loss_numel += n
            for i, (region_name, joint_indices) in enumerate(self.model.regions.items()):
                region_feat = single_region_features[i]
                region_output = self.model.region_models[region_name].decode(tgt_in, region_feat, tgt_mask, tgt_padding_mask, tgt_query_mask=query_mask)
                region_logits = self.model.region_models[region_name].out_proj(region_output).flatten(end_dim=1)
                loss += n * region_loss_weight * F.cross_entropy(region_logits, tgt_out.flatten(), ignore_index=self.pad_id)
                loss_numel += n
