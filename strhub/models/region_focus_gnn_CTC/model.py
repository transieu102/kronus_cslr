...
class RegionGNNCSLRModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #         nn.AvgPool1d(kernel_size=2, stride=2)
        #     )
        self.region_conv = nn.ModuleDict()
        for region_name, joint_indices in self.regions.items():
            self.region_conv[region_name] = nn.Sequential(
                # First conv layer: kernel=5, stride=2, padding=2 to maintain shape
                nn.Conv1d(region_feature_dim, region_feature_dim, kernel_size=5, stride=2, padding=2),
                nn.BatchNorm1d(region_feature_dim),
                nn.GELU(),
                nn.Dropout(dropout),
        ...
        # self.second_out_proj = nn.Linear(d_model, num_classes)
    ...
    def encode(self, poses, return_region_features=False):
        ...
        x = self.temporal_conv(x)  # [B, d_model, T/4]
    ...
    def forward(self, tokenizer: Tokenizer, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        """
        # Encode images
        logits = self.encode(images)  # [B, T/4, d_model]
        logits = self.out_proj(logits)  # [B, T/4, num_classes]
        return logits
