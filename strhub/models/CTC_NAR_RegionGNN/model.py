...
class RegionGNNCSLRModel(nn.Module):
    ...
    def __init__(
        ...
        num_classes_ctc: int = 100,
        num_classes_entropy: int = 100,
        dropout: float = 0.1,
        ...
        **kwargs
        ...
        # self.token_embedding = TokenEmbedding(num_classes_entropy, d_model)
        ...
        self.ctc_proj = nn.Linear(d_model, num_classes_ctc)

        self.cross_attn = nn.MultiheadAttention(d_model, decoder_num_heads, dropout=dropout, batch_first=True)
        self.entropy_proj = nn.Linear(d_model, num_classes_entropy)
        # self.frame_proj = nn.Linear(d_model, num_classes_entropy)
        
        self.num_classes_ctc = num_classes_ctc
        self.num_classes_entropy = num_classes_entropy
    ...
    def encode(self, poses):
        ...
        # Apply temporal pooling first
        x = frame_features.permute(0, 2, 1)  # [B, d_model, T]
        gloss_features = self.temporal_pooling(x)  # [B, d_model, T/4]
        # gloss_features_neighbor_aware = self.temporal_conv(gloss_features)  # [B, d_model, T/4] 2step
        # gloss_features_neighbor_aware = gloss_features_neighbor_aware.permute(0, 2, 1)  # [B, T/4, d_model]
        
        # # Then apply transformer encoding
        # gloss_features = self.pos_encoder(gloss_features)
        # x = self.transformer(x)
        
        # Final MLP
        gloss_features = self.mlp(gloss_features)  # [B, T/4, d_model]
        return gloss_features
    # def decode(
    #     self,
    #     tgt: torch.Tensor,
    #     memory: torch.Tensor,
    ...
    #     # Non-autoregressive decoding - decode all positions at once
    #     tgt_out, sa_weights, ca_weights = self.decode(
    #         tgt_in,
    #         memory,
    ...
    def forward(self, tokenizer: Tokenizer, images: Tensor, mode= 'ctc', max_len=None) -> Tensor:
        # testing = max_length is None
        # max_length = self.max_output_len if max_length is None else min(max_length, self.max_output_len)
        # bs = images.shape[0]
        # +1 for <eos> at end of sequence.
        ...
        memory = self.encode(images)
        if mode == 'ctc':
            logits = self.ctc_proj(memory)
        elif mode == 'entropy':
            step = max_len + 1 if max_len is not None else self.max_output_len + 1
            memory = self.pos_encoder(memory)
            query = self.pos_queries[:, :step].expand(memory.shape[0], -1, -1)
            memory, _ = self.cross_attn(query, memory, memory)
            logits = self.entropy_proj(memory)
        elif mode == 'both':
            logits_ctc = self.ctc_proj(memory)
            logits_entropy = self.entropy_proj(memory)
            return logits_ctc, logits_entropy
        return logits_ctc
